...
import { View, Platform, StyleSheet, Image, Text, requireNativeComponent } from 'react-native'
...
export default class Pdf extends Component {
    ...
    _loadFromSource = (newSource) => {
        ...
        const cacheFile = RNFS.CachesDirectoryPath + '/' + filename

        if (source.cache) {
            RNFS.exists(cacheFile)
                .then((stats) => {
        ...
        try {
            if (source.uri) {
                ...
                if (isNetwork) {
                    ...
                    RNFS.writeFile(cacheFile, data, 'base64')
                        .then(() => {
                            if (this._mounted) {
                                this.setState({ path: cacheFile, isDownloaded: true, progress: 1 })
                            }
